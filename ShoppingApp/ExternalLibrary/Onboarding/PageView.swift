...
public final class PageView: UIView {
    ...
    public var pageButtonTitle: String? { didSet { buttonStart.setTitle(pageButtonTitle, for: .normal) } }
    ...
    fileprivate func setupTopStackView() {
        ...
        //      imageView.setContentHuggingPriority(250.0, for: .vertical)
        imageView.setContentHuggingPriority(UILayoutPriority.init(250.0), for: .vertical)
        titleLabel.setContentHuggingPriority(UILayoutPriority.init(250.0), for: .vertical)
        //      titleLabel.setContentHuggingPriority(252.0, for: .vertical)
    }
    ...
    fileprivate func setupBottomStackView() {
        ...
        if #available(iOS 9.0, *) {
            bottomContainerAnchor = bottomStackView.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -(bottomContainerOffset+(obj_AppDelegate.window?.safeAreaInsets.bottom ?? 0.0)+40))
            bottomContainerHeightAnchor = bottomStackView.heightAnchor.constraint(equalTo: heightAnchor, multiplier: 0.5, constant: -(bottomContainerOffset+(obj_AppDelegate.window?.safeAreaInsets.bottom ?? 0.0)+40) - offsetBetweenContainers / 2)
        } else {
            bottomContainerAnchor = bottomStackView.anchors.bottomAnchor.constraintEqualToAnchor(anchors.bottomAnchor, constant: -(bottomContainerOffset+(obj_AppDelegate.window?.safeAreaInsets.bottom ?? 0.0)+40))
            bottomContainerHeightAnchor = bottomStackView.anchors.heightAnchor.constraintEqualToAnchor(anchors.heightAnchor, multiplier: 0.5, constant: -(bottomContainerOffset+(obj_AppDelegate.window?.safeAreaInsets.bottom ?? 0.0)+40) - offsetBetweenContainers / 2)
        }
        
        if #available(iOS 9.0, *) {
            let bottomAnchors = [
                bottomStackView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: (bottomContainerOffset+40.0)),
                bottomStackView.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -(bottomContainerOffset+40.0)),
            ...
            let bottomAnchors = [
                bottomStackView.anchors.leadingAnchor.constraintEqualToAnchor(anchors.leadingAnchor, constant: (bottomContainerOffset+40.0)),
                bottomStackView.anchors.trailingAnchor.constraintEqualToAnchor(anchors.trailingAnchor, constant: -(bottomContainerOffset+40.0)),
                bottomContainerAnchor,
                bottomContainerHeightAnchor
            ].flatMap { $0 }
            NSLayoutConstraint.activate(bottomAnchors)
        ...
        descriptionLabel.numberOfLines = 0
            ...
            bottomStackView.addArrangedSubview(buttonStart)
            buttonStart.setTitle(pageButtonTitle, for: .normal)
            buttonStart.titleLabel?.font = UIFont.systemFont(ofSize: 17)
            buttonStart.titleLabel?.textAlignment = .center
            buttonStart.layer.cornerRadius = 20
            buttonStart.setUpGradientLayer(colors: [UIColor.hexStringToUIColor(hex: "0093E9"),
                                                    UIColor.hexStringToUIColor(hex: "80D0C7")])
        buttonStart.addTarget(self, action: #selector(self.startButtonAction(_:)), for: .touchUpInside)
    }
    
    @objc func startButtonAction(_ sender: UIButton) {
        myApp.window?.rootViewController?.view.endEditing(true)
...
extension PageView: Configurable {
    public func configure(_ configuration: OnboardingConfiguration) {
        ...
        pageButtonTitle = configuration.pageButtonTitle
