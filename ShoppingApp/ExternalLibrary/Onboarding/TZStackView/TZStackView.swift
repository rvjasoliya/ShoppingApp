...
open class TZStackView: UIView {
    ...
    fileprivate func createFillEquallyConstraints(_ views: [UIView], priority: Float = 1000) -> [NSLayoutConstraint] {
        switch axis {
        case .horizontal:
            return equalAttributes(views.filter({ !self.isHidden($0) }), attribute: .width, priority: priority)
            
        case .vertical:
            return equalAttributes(views.filter({ !self.isHidden($0) }), attribute: .height, priority: priority)
        }
    }
    ...
    fileprivate func createMatchEdgesContraints(_ views: [UIView]) -> [NSLayoutConstraint] {
        ...
        case .horizontal:
            switch alignment {
            case .fill:
                constraints += equalAttributes(views, attribute: .bottom)
                constraints += equalAttributes(views, attribute: .top)
            case .center:
                constraints += equalAttributes(views, attribute: .centerY)
            case .leading, .top:
                constraints += equalAttributes(views, attribute: .top)
            case .trailing, .bottom:
                constraints += equalAttributes(views, attribute: .bottom)
            case .firstBaseline:
                constraints += equalAttributes(views, attribute: .firstBaseline)
            }
            
        case .vertical:
            switch alignment {
            case .fill:
                constraints += equalAttributes(views, attribute: .leading)
                constraints += equalAttributes(views, attribute: .trailing)
            case .center:
                constraints += equalAttributes(views, attribute: .centerX)
            case .leading, .top:
                constraints += equalAttributes(views, attribute: .leading)
            case .trailing, .bottom:
                constraints += equalAttributes(views, attribute: .trailing)
